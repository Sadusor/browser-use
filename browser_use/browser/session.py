...
class BrowserSession(BaseModel):
	...
	async def connect(self, cdp_url: str | None = None) -> Self:
		...
		try:
			...
			self._register_fetch_request_gate()
	...
	def _register_fetch_request_gate(self) -> None:
		"""Register one authoritative Fetch owner for request policy + proxy auth.

		The cdp-use registry is single-slot per protocol event, so ORION must not
		install a second independent requestPaused callback. This handler owns every
		continue/fail decision for Fetch-paused requests and preserves proxy auth.
		"""
		assert self._cdp_client_root is not None

		def _on_request_paused(event: RequestPausedEvent, session_id: SessionID | None = None):
			create_task_with_error_handling(
				self._handle_fetch_request_paused(event, session_id),
				name='fetch_request_policy',
		...
		def _on_auth_required(event: AuthRequiredEvent, session_id: SessionID | None = None):
			create_task_with_error_handling(
				self._handle_fetch_auth_required(event, session_id),
				name='fetch_proxy_auth',
				logger_instance=self.logger,
				suppress_exceptions=True,
			)
		...
		self.logger.debug('Registered unified Fetch request-policy/proxy-auth handlers')

	async def _handle_fetch_request_paused(
		self, event: RequestPausedEvent, session_id: SessionID | None = None
	) -> None:
		"""Fail closed on forbidden request URLs before network transmission."""
		assert self._cdp_client_root is not None
		...
		try:
			policy_configured = bool(
				self.browser_profile.allowed_domains or self.browser_profile.prohibited_domains
			)
			if policy_configured:
				# A configured policy without an attached policy engine is a fail-closed error.
				allowed = bool(
					self._security_watchdog is not None
					and self._security_watchdog._is_url_allowed(url)
				)
			else:
				allowed = True

			if not allowed:
				self.logger.warning(f'⛔️ Fetch gate blocked request before transmission: {url}')
				await self._cdp_client_root.send.Fetch.failRequest(
			...
			# Never recover from a policy-handler failure by releasing the request.
			self.logger.error(
				f'Fetch request gate failed for {url!r}; failing closed: {type(e).__name__}: {e}'
			)
			try:
				await self._cdp_client_root.send.Fetch.failRequest(
					params={'requestId': request_id, 'errorReason': 'BlockedByClient'},
				...
				self.logger.debug(
					f'Fetch fail-closed cleanup could not fail request {request_id}: '
					f'{type(fail_error).__name__}: {fail_error}'
				)

	async def _handle_fetch_auth_required(
		self, event: AuthRequiredEvent, session_id: SessionID | None = None
	) -> None:
		"""Preserve proxy authentication under the same authoritative Fetch owner."""
		assert self._cdp_client_root is not None
		request_id = event.get('requestId') or event.get('request_id')
		if not request_id:
			return
		...
		try:
			...
			self.logger.debug(f'Fetch auth response failed: {type(e).__name__}: {e}')

	async def _setup_proxy_auth(self) -> None:
		"""Compatibility hook; unified Fetch handlers now own proxy auth.

		Fetch is enabled per attached target by SessionManager before a paused target
		is released. Keeping this hook avoids changing the surrounding lifecycle.
		"""
		if self.browser_profile.proxy and self.browser_profile.proxy.username:
			self.logger.debug('Proxy auth is handled by the unified Fetch request gate')
